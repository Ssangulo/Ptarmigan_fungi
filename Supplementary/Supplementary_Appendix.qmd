...
**Most dark taxa are close to known references, but a substantial minority are genuinely novel.** Of the 493 dark OTUs, 143 are placeable to an existing, named UNITE SH (each resolving to a stable SH page, `https://unite.ut.ee/sh/<code>`, that carries the taxon name and its DOI), 247 form new species hypotheses, and 90 have no SH match at all (>20% divergent from any reference); 13 were dropped by ITSx as chimeric or non-ITS. The best-hit similarity distribution (Figure S-E1a) shows the dark fraction is bimodal — a spike near 100% (OTUs essentially identical to a reference but withheld a genus by the conservative `minBoot = 80` threshold) plus a real tail of divergent, candidate-novel lineages.
...
**E1 interpretation.** SH matching directly addresses the "winter = dark taxa" caveat: of the winter dark-read burden, **33.5% is placeable to an existing UNITE SH**, so a third of winter's otherwise-unnamed reads gain a species hypothesis, while the remainder forms candidate new SHs. Winter's dominant dark OTU, **OTU1369**, is *genuinely novel* — it forms a new SH only 85.4% similar to any reference and resolves no further than class Dothideomycetes — so it represents a candidate new, cold-associated lineage rather than a mere database gap. Two caveats: (i) OTU320, previously listed among winter's dark dominants, is an ITSx-flagged chimeric/broken sequence and is dropped here; (ii) these placements are exploratory — placeable taxa carry citable UNITE SH identifiers, and the novel taxa can be given permanent DOIs by imprinting them via PlutoF (sequence export prepared in `models/E1_novel_taxa_for_plutof.fasta`).
:::
